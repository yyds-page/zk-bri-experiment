...
def filter_coeffs(ratio):
    ...
    return [int(2 ** (NBITS + 4) * math.sin(math.pi * pt) * math.sin(math.pi / 2 * pt) / (
            math.pi ** 2 / 2 * pt ** 2) / ratio) << NBITS for pt in
            [offset + idx * spacing for idx in range(0, 2 * ratio)]]
...
def print_pws(ratio, fh):
    ...
    coeffs_idx = list(range(voffset - 2 * ratio, voffset))
